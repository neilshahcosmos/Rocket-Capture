...
# # %%
# t0G = time.time()
# print("Hello! Now Beginning Zoomed In Rocket Evolution")
...
# xC = getHaloC(tmax)
# xArr[:, :, 0] = np.asarray([xC + L * (2*np.random.rand(3)-1) for i in range(0, nRockets)])
...
haloXArr = np.asarray([getHaloC(t) for t in range(tmax)])
np.save(simDir + "HaloCoords.npy", haloXArr)
